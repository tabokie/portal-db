...
Status ServerImpl::Open(std::string s) {
  pstore = std::make_unique<PersistHashTrie>(s);
  return Status::OK();
}

Status ServerImpl::Serve(std::string ip, std::string port) {
...
Status ServerImpl::Close() {
  closesocket(server_);
  server_ = INVALID_SOCKET;
  return Status::OK();
}

Status ServerImpl::InitSocket(std::string addr, std::string port) {
  server_ = INVALID_SOCKET;
  if(!WSAProof::Ok()) {
...
void ServerImpl::Spawn(SOCKET socket) {
...
bool ServerImpl::Send(SOCKET target, std::string message) {
...
bool ServerImpl::Send(SOCKET target, const KeyValue& data) {
...
bool ServerImpl::Send(SOCKET target, const Key& key, const Value& value) {
